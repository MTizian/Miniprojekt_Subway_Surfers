...
int checkCollision(position *train_1, position *train_2, position *train_3, position *player) {
	...
	//Setze positionen der Z�ge auf eins
	for (int i = 0; i < train_len_y; i++){
		for (int j = 0; j < train_len_x; j++){
...
int mainGame() {
	...
	//Wartezeit bis zur n�chsten spielschleife
	int duration = 200;
	...
	while (user_input != ESC_KEY) {
		...
		//Z�ge PLatzieren auf der game map
		//       Zielort   was		L�nge x			L�nge y			Farbe
		...
		iterations++;
		if (iterations == 10) {
			//F�r 10 Iterationen erh�ht sich der score
			score++;
			//Setze iterationen zur�ck
			iterations = 0;
		}
...
void loadingAnimation() {
	...
	//Loading animation
	printf("|");
	//Schleife f�r das erstellen von farbigen Pixeln
	for (int i = 0; i < 15; i++) {
		Sleep(50);
...
void setEndMenu(position feld[SCREEN_HEIGHT][SCREEN_WIDTH], int *score) {
	...
	//�hnlich wie printf nur das die formatierte zeichenkette nicht ausgegeben wird sondern in einer anderen gespeichert wird
	//Dabei wird der score an den bereits vorhandenen string drangesetzt
	//	     Zielort	 gr��e des Array	format			  %d
	sprintf_s(scoreText, sizeof(scoreText), "Your Score: %d", *score);
	//anh�ngen von einer Zeichenkette in dem Fall ein Leerzeichen
